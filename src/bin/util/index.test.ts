...
describe("bin util functions", () => {
	...
	describe("getCopyCommand", () => {
		...
		test("Should return the copy command for Mac OSX", () => {
			...
			expect(getCopyCommand(SUPPORTED_PLATFORMS.DARWIN)).toEqual(
				COPY.DARWIN
			);
		});
		...
		test("Should return the copy command for Linux", () => {
			...
			expect(getCopyCommand(SUPPORTED_PLATFORMS.LINUX)).toEqual(
				COPY.LINUX
			);
		...
		test("Should return the copy command for Windows", () => {
			...
			expect(getCopyCommand(SUPPORTED_PLATFORMS.WIN32)).toEqual(
				COPY.WIN32
			);
		});
	});
